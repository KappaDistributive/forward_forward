...
class Layer(nn.Linear):
    def __init__(self, in_features: int, out_features: int, bias: bool, lr: float, threshold: float, device: Optional[torch.device] = None,
                 dtype=None) -> None:
        super().__init__(in_features=in_features, out_features=out_features, bias=bias, device=device, dtype=dtype)
        self.layer_norm = nn.LayerNorm(normalized_shape=in_features, elementwise_affine=False, device=device, dtype=dtype)
        self.activation = nn.ReLU()
        self.optimizer = torch.optim.Adam(self.parameters(), lr=lr)
        self.threshold = threshold

    def forward(self, x: Tensor) -> Tensor:
        # x_direction = self.layer_norm(x)
...
class Net(nn.Module):
    def __init__(self, dimensions: list[int], lr : float, threshold: float, device: Optional[torch.device] = None, dtype=None):
        super().__init__()
        self.layers: list[Layer] = []
        for index in range(len(dimensions) - 1):
            self.layers.append(Layer(dimensions[index], dimensions[index + 1], bias=True, lr=lr, threshold=threshold, device=device, dtype=dtype))
    ...
    def train(self, x: Tensor, y: Tensor):
        ...
        h_pos, h_neg = torch.flatten(x_positive, 1), torch.flatten(x_negative, 1)
        for index, layer in enumerate(self.layers):
            h_pos, h_neg, _ = layer.train(h_pos, h_neg)
...
if __name__ == "__main__":
    ...
    net = Net([28*28, 500, 500, 500, 500, 500, 500], lr=0.03, threshold=2.)
